...
import { SettingsIcon, LogOutIcon, PowerIcon, SideIcons } from "../raw_components/raw_icons";
...
export const UserSideBar = () => {
  ...
  return (
    ...
    <SideIcons className="w-5/6 h-16 " />
      ...
      <ProfileCard user={{ name: userEmail, email: "johnDoe@gmail.com", friends: 10, strangersMet: 5 }} />
          ...
          <div>
            ...
            <div className="grid gap-2 overflow-y-auto max-h-32">
              {recentlyChatData.map((chat) => (
                <div key={chat.id} className="flex items-center text-sm rounded-md bg-[#ededed] p-1 pl-4">
                  {/* <AvatarComponent
                    src={chat.avatar}
      ...
      <CardFooter className="w-full border-t p-6 flex justify-between absolute bottom-0">
        ...
        <div className="flex items-center gap-2">
          <Button variant="primary" size="icon" className="rounded-full border-primary">
            <LogOutIcon className="h-4 w-4" />
            <span className="sr-only" onClick={() => handleLogout()}>Log out</span>
          </Button>
